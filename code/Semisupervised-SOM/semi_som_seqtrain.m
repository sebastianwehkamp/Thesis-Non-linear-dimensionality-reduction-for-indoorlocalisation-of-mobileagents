...
for t = 1:trainlen, 
  ...
  factor1 = 1.0;
  factor2 = 3.0;
%   For loop with distance measurement check  
%   for i=1:length(M)
%      % Compute term1
%      term1 = h(i) * (M(i,known)-x);
%      dist_term1_old = pdist([M(i,known); x]);
%      M(i,known) = M(i,known) - factor1*term1;
%      dist_term1_new = pdist([M(i,known); x]);
%      dist_dif1 = dist_term1_old - dist_term1_new;
%      
%      % Compute term2
%      term2 = 2*h(i)*(M(i,known)-P(bmu,:));
%      dist_term2_old = pdist([M(i,known); P(bmu,:)]);
%      M(i,known) = M(i,known) - factor2*term2;
%      dist_term2_new = pdist([M(i,known); P(bmu,:)]);
%      dist_dif2 = dist_term2_old - dist_term2_new;
%   end
  

% Clean for loop without neigborhood influence
%     for i = 1:length(M)
%      % Compute term1
%      term1 = h(i) * (M(i,known) - x);
%      M(i,known) = M(i,known) - factor1*term1;
...
%      % Compute term2
%      if i==bmu
%          term2 = 2 * alfa(ind) *(M(i,known) - P(bmu,:));
%          M(i,known) = M(i,known) - factor2 * term2;
%      end
%     end
...
%   Clean for loop with neighborhood influence
    for i = 1:length(M)
     % Compute term1
     term1 = h(i) * (M(i,known) - x);
     M(i,known) = M(i,known) - factor1*term1;
...
% Clean for loop with just second term
%     for i = 1:length(M)
%      % Compute term2
%      term2 = 2 * h(i) * (M(i,known) - P(bmu,:));
%      M(i,known) = M(i,known) - factor2 * term2;
