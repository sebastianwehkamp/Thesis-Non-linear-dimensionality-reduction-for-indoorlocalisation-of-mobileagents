function [sMap, sTrain] = som_seqtrain(sMap, D, P, varargin)
...
for t = 1:trainlen, 
  ...
  % Dx contains w_old - x, h the neighborhood information*learningrate, and
  % P{bmu} the known wifi info for the BMU
  term1 = h(:,ones(sum(known),1)).*Dx;
  term2 = 2.*h(:,ones(sum(known),1)).*(M(:,known)- P{bmu}(mu_x_1,known));
  
  % Check if it actually moves closer...
  for (i=1:length(M))
      dist_term1_old = pdist2(M(i,known), x);
      dist_term1_new = pdist2(M(i,known)- term1(i), x);
      dist_dif1 = dist_term1_old - dist_term1_new;
      
      dist_term2_old = pdist2(M(i,known), P{bmu});
      dist_term2_new = pdist2(M(i,known) - term2(i), P{bmu});
      dist_dif2 = dist_term2_old - dist_term2_new;
  end
  
  % Original rule: M(:,known) = M(:,known) - h(:,ones(sum(known),1)).*Dx;
  M(:,known) = M(:,known) - (factor1.*term1 + factor2.*term2);
