function [sMap, sTrain] = som_seqtrain_lvq(sMap, D, lvq_omegas, lvq_weights, varargin)
...
for t = 1:trainlen, 
  ...
  minDist = inf;
  prototype = 0;            
  for i = 1:size(lvq_weights)
      dist = (x - lvq_weights(i,:))*lvq_omegas{i}'*lvq_omegas{i}*(x - lvq_weights(i,:))';
      if dist<minDist
          minDist = dist;
          prototype = i;
      end
 ...
 %Dx = M(:,known) - x(mu_x_1,known);     % each map unit minus the vector
  ...
  % update M
  for i=1:length(M)
     %distOld = (x - M(i,known))*omega'*omega*(x-M(i,known))';
     M(i,known) = M(i,known) - h(i) * (-2 * omega * omega' * (x-M(i,known))')';
     %distNew = (x - M(i,known))*omega'*omega*(x-M(i,known))';
     %distDif = distOld - distNew;
  end
  
 %M(:,known) = M(:,known) - h(:,ones(sum(known),1)).*(Dx*-2*omega);
